  ...
  55  0000 cd0871        	call	L3_M24LR04E_LowLevel_DeInit
  57                     ; 88 }
  60  0003 81            	ret
  ...
  95  0004 cd089d        	call	L5_M24LR04E_LowLevel_Init
...
1388  04c9 81            	ret
1468                     ; 458 void M24LR04E_ReadBuffer (uint8_t EE_address, uint16_t ReadAddr, uint8_t NumByteToRead,uint8_t* pBuffer)
1468                     ; 459 {
1469                     	switch	.text
1470  04ca               _M24LR04E_ReadBuffer:
1472  04ca 88            	push	a
1473  04cb 5204          	subw	sp,#4
1474       00000004      OFST:	set	4
1477                     ; 460 int32_t I2C_TimeOut = I2C_TIMEOUT;
1479  04cd ae00ff        	ldw	x,#255
1480  04d0 1f03          	ldw	(OFST-1,sp),x
1481  04d2 ae0000        	ldw	x,#0
1482  04d5 1f01          	ldw	(OFST-3,sp),x
1483                     ; 464   I2C_AcknowledgeConfig(M24LR04E_I2C, ENABLE);
1485  04d7 4b01          	push	#1
1486  04d9 ae5210        	ldw	x,#21008
1487  04dc cd0000        	call	_I2C_AcknowledgeConfig
1489  04df 84            	pop	a
1490                     ; 468   I2C_GenerateSTART(M24LR04E_I2C, ENABLE);
1492  04e0 4b01          	push	#1
1493  04e2 ae5210        	ldw	x,#21008
1494  04e5 cd0000        	call	_I2C_GenerateSTART
1496  04e8 84            	pop	a
1498  04e9               L365:
1499                     ; 471   while (!I2C_CheckEvent(M24LR04E_I2C, I2C_EVENT_MASTER_MODE_SELECT)&& I2C_TimeOut-->0)  // EV5 
...
1523  050d               L765:
1524                     ; 476   I2C_Send7bitAddress(M24LR04E_I2C, EE_address , I2C_Direction_Transmitter);
...
1532  0518 85            	popw	x
1534  0519               L375:
1535                     ; 479   while (!I2C_CheckEvent(M24LR04E_I2C, I2C_EVENT_MASTER_TRANSMITTER_MODE_SELECTED)&& I2C_TimeOut-->0) // EV6 
...
1559  053d               L775:
1560                     ; 484 	I2C_SendData(M24LR04E_I2C,(uint8_t)(ReadAddr >> 8)); // MSB 
...
1567  0546 84            	pop	a
1569  0547               L306:
1570                     ; 489   while (!I2C_CheckEvent(M24LR04E_I2C, I2C_EVENT_MASTER_BYTE_TRANSMITTED)&& I2C_TimeOut-->0) // EV8 
...
1594  056b               L706:
1595                     ; 494 	I2C_SendData(M24LR04E_I2C,(uint8_t)ReadAddr); // LSB 
...
1602  0574 84            	pop	a
1604  0575               L316:
1605                     ; 497   while (!I2C_CheckEvent(M24LR04E_I2C, I2C_EVENT_MASTER_BYTE_TRANSMITTED)&& I2C_TimeOut-->0) // EV8 
...
1629  0599               L716:
1630                     ; 503   I2C_GenerateSTART(M24LR04E_I2C, ENABLE);
1632  0599 4b01          	push	#1
1633  059b ae5210        	ldw	x,#21008
1634  059e cd0000        	call	_I2C_GenerateSTART
1636  05a1 84            	pop	a
1638  05a2               L326:
1639                     ; 506   while (!I2C_CheckEvent(M24LR04E_I2C, I2C_EVENT_MASTER_MODE_SELECT)&& I2C_TimeOut-->0)  // EV5 
...
1663  05c6               L726:
1664                     ; 511   I2C_Send7bitAddress(M24LR04E_I2C, EE_address, I2C_Direction_Receiver);
...
1672  05d1 85            	popw	x
1674  05d2               L336:
1675                     ; 514   while (!I2C_CheckEvent(M24LR04E_I2C, I2C_EVENT_MASTER_RECEIVER_MODE_SELECTED)&& I2C_TimeOut-->0)  // EV6 
...
1700  05f8               L156:
1701                     ; 524 		while (!I2C_CheckEvent(M24LR04E_I2C, I2C_EVENT_MASTER_BYTE_RECEIVED)&& I2C_TimeOut-->0)  
...
1808  067d 81            	ret
1850                     ; 565 uint16_t M24LR04E_ReadTemp(void)
1850                     ; 566 {
1851                     	switch	.text
1852  067e               _M24LR04E_ReadTemp:
1854  067e 89            	pushw	x
1855       00000002      OFST:	set	2
1858                     ; 567   __IO uint16_t RegValue = 0;
1860  067f 5f            	clrw	x
1861  0680 1f01          	ldw	(OFST-1,sp),x
1862                     ; 570   I2C_AcknowledgeConfig(M24LR04E_I2C, ENABLE);
1864  0682 4b01          	push	#1
1865  0684 ae5210        	ldw	x,#21008
1866  0687 cd0000        	call	_I2C_AcknowledgeConfig
1868  068a 84            	pop	a
1869                     ; 574   I2C_GenerateSTART(M24LR04E_I2C, ENABLE);
1871  068b 4b01          	push	#1
1872  068d ae5210        	ldw	x,#21008
1873  0690 cd0000        	call	_I2C_GenerateSTART
1875  0693 84            	pop	a
1877  0694               L517:
1878                     ; 577   while (!I2C_CheckEvent(M24LR04E_I2C, I2C_EVENT_MASTER_MODE_SELECT))  /* EV5 */
...
1888                     ; 582   I2C_Send7bitAddress(M24LR04E_I2C, M24LR04E_ADDR, I2C_Direction_Transmitter);
...
1895  06ac 85            	popw	x
1897  06ad               L327:
1898                     ; 585   while (!I2C_CheckEvent(M24LR04E_I2C, I2C_EVENT_MASTER_TRANSMITTER_MODE_SELECTED)) /* EV6 */
...
1908                     ; 590   I2C_SendData(M24LR04E_I2C, M24LR04E_REG_TEMP);
1910  06bb 4b00          	push	#0
1911  06bd ae5210        	ldw	x,#21008
1912  06c0 cd0000        	call	_I2C_SendData
1914  06c3 84            	pop	a
1916  06c4               L137:
1917                     ; 593   while (!I2C_CheckEvent(M24LR04E_I2C, I2C_EVENT_MASTER_BYTE_TRANSMITTED)) /* EV8 */
...
1927                     ; 599   I2C_GenerateSTART(M24LR04E_I2C, ENABLE);
1929  06d2 4b01          	push	#1
1930  06d4 ae5210        	ldw	x,#21008
1931  06d7 cd0000        	call	_I2C_GenerateSTART
1933  06da 84            	pop	a
1935  06db               L737:
1936                     ; 602   while (!I2C_CheckEvent(M24LR04E_I2C, I2C_EVENT_MASTER_MODE_SELECT))  /* EV5 */
...
1946                     ; 607   I2C_Send7bitAddress(M24LR04E_I2C, M24LR04E_ADDR, I2C_Direction_Receiver);
...
1953  06f3 85            	popw	x
1955  06f4               L547:
1956                     ; 610   while (!I2C_CheckEvent(M24LR04E_I2C, I2C_EVENT_MASTER_RECEIVER_MODE_SELECTED))  /* EV6 */
...
1967  0702               L357:
1968                     ; 615   while (!I2C_CheckEvent(M24LR04E_I2C, I2C_EVENT_MASTER_BYTE_RECEIVED))  /* EV7 */
...
1978                     ; 620   RegValue = I2C_ReceiveData(M24LR04E_I2C) << 8;
...
2014                     ; 633   RegValue |= I2C_ReceiveData(M24LR04E_I2C);
...
2037  0755 81            	ret
2080                     ; 644 uint8_t M24LR04E_ReadConfReg(void)
2080                     ; 645 {
2081                     	switch	.text
2082  0756               _M24LR04E_ReadConfReg:
2084  0756 88            	push	a
2085       00000001      OFST:	set	1
2088                     ; 646   __IO uint8_t RegValue = 0;
2090  0757 0f01          	clr	(OFST+0,sp)
2091                     ; 649   I2C_AcknowledgeConfig(M24LR04E_I2C, ENABLE);
2093  0759 4b01          	push	#1
2094  075b ae5210        	ldw	x,#21008
2095  075e cd0000        	call	_I2C_AcknowledgeConfig
2097  0761 84            	pop	a
2098                     ; 652   I2C_GenerateSTART(M24LR04E_I2C, ENABLE);
2100  0762 4b01          	push	#1
2101  0764 ae5210        	ldw	x,#21008
2102  0767 cd0000        	call	_I2C_GenerateSTART
2104  076a 84            	pop	a
2106  076b               L5001:
2107                     ; 655   while (!I2C_CheckEvent(M24LR04E_I2C, I2C_EVENT_MASTER_MODE_SELECT))  /* EV5 */
...
2117                     ; 660   I2C_Send7bitAddress(M24LR04E_I2C, M24LR04E_ADDR, I2C_Direction_Transmitter);
...
2124  0783 85            	popw	x
2126  0784               L3101:
2127                     ; 663   while (!I2C_CheckEvent(M24LR04E_I2C, I2C_EVENT_MASTER_TRANSMITTER_MODE_SELECTED)) /* EV6 */
...
2137                     ; 668   I2C_SendData(M24LR04E_I2C, M24LR04E_REG_CONF);
2139  0792 4b01          	push	#1
2140  0794 ae5210        	ldw	x,#21008
2141  0797 cd0000        	call	_I2C_SendData
2143  079a 84            	pop	a
2145  079b               L1201:
2146                     ; 671   while (!I2C_CheckEvent(M24LR04E_I2C, I2C_EVENT_MASTER_BYTE_TRANSMITTED)) /* EV8 */
...
2156                     ; 677   I2C_GenerateSTART(M24LR04E_I2C, ENABLE);
2158  07a9 4b01          	push	#1
2159  07ab ae5210        	ldw	x,#21008
2160  07ae cd0000        	call	_I2C_GenerateSTART
2162  07b1 84            	pop	a
2164  07b2               L7201:
2165                     ; 680   while (!I2C_CheckEvent(M24LR04E_I2C, I2C_EVENT_MASTER_MODE_SELECT))  /* EV5 */
...
2175                     ; 685   I2C_Send7bitAddress(M24LR04E_I2C, M24LR04E_ADDR, I2C_Direction_Receiver);
...
2182  07ca 85            	popw	x
2184  07cb               L5301:
2185                     ; 688   while (!I2C_CheckEvent(M24LR04E_I2C, I2C_EVENT_MASTER_RECEIVER_MODE_SELECTED))  /* EV6 */
...
2221                     ; 702   RegValue = I2C_ReceiveData(M24LR04E_I2C);
2223  07f9 ae5210        	ldw	x,#21008
2224  07fc cd0000        	call	_I2C_ReceiveData
2226  07ff 6b01          	ld	(OFST+0,sp),a
2227                     ; 705   return (RegValue);
2229  0801 7b01          	ld	a,(OFST+0,sp)
2232  0803 5b01          	addw	sp,#1
2233  0805 81            	ret
2273                     ; 714 void M24LR04E_WriteConfReg(uint8_t RegValue)
2273                     ; 715 {
2274                     	switch	.text
2275  0806               _M24LR04E_WriteConfReg:
2277  0806 88            	push	a
2278       00000000      OFST:	set	0
2281                     ; 718   I2C_GenerateSTART(M24LR04E_I2C, ENABLE);
2283  0807 4b01          	push	#1
2284  0809 ae5210        	ldw	x,#21008
2285  080c cd0000        	call	_I2C_GenerateSTART
2287  080f 84            	pop	a
2289  0810               L7601:
2290                     ; 721   while (!I2C_CheckEvent(M24LR04E_I2C, I2C_EVENT_MASTER_MODE_SELECT))  /* EV5 */
...
2300                     ; 726   I2C_Send7bitAddress(M24LR04E_I2C, M24LR04E_ADDR, I2C_Direction_Transmitter);
...
2307  0828 85            	popw	x
2309  0829               L5701:
2310                     ; 729   while (!I2C_CheckEvent(M24LR04E_I2C, I2C_EVENT_MASTER_TRANSMITTER_MODE_SELECTED)) /* EV6 */
...
2320                     ; 734   I2C_SendData(M24LR04E_I2C, M24LR04E_REG_CONF);
2322  0837 4b01          	push	#1
2323  0839 ae5210        	ldw	x,#21008
2324  083c cd0000        	call	_I2C_SendData
2326  083f 84            	pop	a
2328  0840               L3011:
2329                     ; 737   while (!I2C_CheckEvent(M24LR04E_I2C, I2C_EVENT_MASTER_BYTE_TRANSMITTED)) /* EV8 */
...
2339                     ; 742   I2C_SendData(M24LR04E_I2C, RegValue);
...
2346  0857 84            	pop	a
2348  0858               L1111:
2349                     ; 745   while (!I2C_CheckEvent(M24LR04E_I2C, I2C_EVENT_MASTER_BYTE_TRANSMITTED)) /* EV8 */
...
2359                     ; 750   I2C_GenerateSTOP(M24LR04E_I2C, ENABLE);
...
2369  086f 84            	pop	a
2370  0870 81            	ret
2398                     ; 758 static void M24LR04E_LowLevel_DeInit(void)
2398                     ; 759 {
2399                     	switch	.text
2400  0871               L3_M24LR04E_LowLevel_DeInit:
2404                     ; 761   I2C_Cmd(M24LR04E_I2C, DISABLE);
2406  0871 4b00          	push	#0
2407  0873 ae5210        	ldw	x,#21008
2408  0876 cd0000        	call	_I2C_Cmd
2410  0879 84            	pop	a
2411                     ; 763   I2C_DeInit(M24LR04E_I2C);
2413  087a ae5210        	ldw	x,#21008
2414  087d cd0000        	call	_I2C_DeInit
2416                     ; 766   CLK_PeripheralClockConfig(M24LR04E_I2C_CLK, DISABLE);
2418  0880 ae0300        	ldw	x,#768
2419  0883 cd0000        	call	_CLK_PeripheralClockConfig
2421                     ; 769   GPIO_Init(M24LR04E_I2C_SCL_GPIO_PORT, M24LR04E_I2C_SCL_PIN, GPIO_Mode_In_PU_No_IT);
2423  0886 4b40          	push	#64
2424  0888 4b02          	push	#2
2425  088a ae500a        	ldw	x,#20490
2426  088d cd0000        	call	_GPIO_Init
2428  0890 85            	popw	x
2429                     ; 772   GPIO_Init(M24LR04E_I2C_SDA_GPIO_PORT, M24LR04E_I2C_SDA_PIN, GPIO_Mode_In_PU_No_IT);
...
2437                     ; 774 }
2440  089c 81            	ret
2465                     ; 781 static void M24LR04E_LowLevel_Init(void)
2465                     ; 782 {
2466                     	switch	.text
2467  089d               L5_M24LR04E_LowLevel_Init:
2471                     ; 784   CLK_PeripheralClockConfig(M24LR04E_I2C_CLK, ENABLE);
2473  089d ae0301        	ldw	x,#769
2474  08a0 cd0000        	call	_CLK_PeripheralClockConfig
2476                     ; 786 }
2479  08a3 81            	ret
2492                     	xdef	_M24LR04E_WriteOneByte
2493                     	xdef	_M24LR04E_ReadBuffer
2494                     	xdef	_M24LR04E_ReadOneByte
2495                     	xdef	_M24LR04E_WriteConfReg
2496                     	xdef	_M24LR04E_ReadConfReg
2497                     	xdef	_M24LR04E_WriteReg
2498                     	xdef	_M24LR04E_ReadReg
2499                     	xdef	_M24LR04E_ReadTemp
2500                     	xdef	_M24LR04E_GetStatus
2501                     	xdef	_M24LR04E_Init
2502                     	xdef	_M24LR04E_DeInit
